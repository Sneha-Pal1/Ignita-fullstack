...
import { activityFeed } from '@/lib/data/analytics';
      ...
      <div className="p-6 bg-zinc-900 border border-zinc-800 rounded-xl space-y-4">
        {activityFeed.map((item, index) => (
          <div key={item.id} className="flex gap-4">
            {/* Icon */}
            <div
              className={`flex-shrink-0 w-10 h-10 rounded-lg ${getActionBg(item.action)} flex items-center justify-center ${getActionColor(item.action)}`}
            >
