...
export const toneOptions = [
  ...
  { value: "excited", label: "Excited", description: "Enthusiastic with emojis" },
  { value: "minimal", label: "Minimal", description: "Brief and concise" },
  { value: "storytelling", label: "Storytelling", description: "Personal narrative" },
...
export function generateHashtags(achievementType: string, skills: string[]): string[] {
  const baseHashtags: Record<string, string[]> = {
    hackathon: ["#Hackathon", "#Innovation", "#BuildersLife", "#CodeCreativity"],
    internship: ["#Internship", "#CareerGrowth", "#InternsOfLinkedIn", "#LearningJourney"],
    achievement: ["#Proud", "#Achievement", "#Growth", "#Grateful"],
    certification: ["#Learning", "#ProfessionalDevelopment", "#Certified", "#GrowthMindset"],
    workshop: ["#ContinuousLearning", "#Development", "#NeverStopLearning", "#KnowledgeSharing"],
    project: ["#ProjectWork", "#Development", "#BuildInPublic", "#Innovation"],
  };

  const typeHashtags = baseHashtags[achievementType.toLowerCase()] || baseHashtags.achievement;
...
export function generateMockPost(
  ...
  const achievementTypeKey = achievementType.toLowerCase() as keyof typeof linkedinPostTemplates;
  const toneKey = tone as keyof (typeof linkedinPostTemplates)[keyof typeof linkedinPostTemplates];

  const template =
    linkedinPostTemplates[achievementTypeKey]?.[toneKey] || linkedinPostTemplates.achievement.professional;
